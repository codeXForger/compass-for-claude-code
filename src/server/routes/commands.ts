...
function commandsDir(ctx: ServerContext, scope: Scope): string {
  const p = resolvePaths(ctx.cwd);
  return scope === "project"
    ? join(p.projectClaudeDir, "commands")
    : join(p.userClaudeDir, "commands");
}

function listScope(ctx: ServerContext, scope: Scope) {
  const dir = commandsDir(ctx, scope);
...
export function commandsRoute(ctx: ServerContext) {
  const app = new Hono();
  ...
  app.get("/:scope/:name", (c) => {
    ...
    const file = resolveWithin(commandsDir(ctx, scope), `${c.req.param("name")}.md`);
